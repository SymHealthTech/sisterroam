...
const ROLE_ACTIONS = {
  guest: [
    { icon: Search,  title: 'Find a host',      desc: 'Browse verified hosts in your destination',   href: '/explore'   },
    { icon: Shield,  title: 'Get verified',      desc: 'Verify your ID to unlock more hosts',          href: '/profile'   },
    { icon: Users,   title: 'Join the community',desc: 'Connect with sister travellers worldwide',     href: '/community' },
  ],
  host: [
    { icon: Home,    title: 'Set up hosting',    desc: 'Create your hosting profile and availability', href: '/profile'   },
    { icon: Shield,  title: 'Get verified',       desc: 'Build trust with ID and background checks',   href: '/profile'   },
    { icon: Users,   title: 'Meet the community', desc: 'Explore the SisterRoam community feed',       href: '/community' },
  ],
  both: [
    { icon: Search,  title: 'Find a host',        desc: 'Browse verified hosts in your destination',  href: '/explore'   },
    { icon: Home,    title: 'Set up hosting',      desc: 'Open your home to sister travellers',        href: '/profile'   },
    { icon: MapPin,  title: 'Explore the world',   desc: 'See where the community is heading',         href: '/community' },
  ],
}
      ...
      <div className="w-full max-w-lg">
        ...
        <div className={cn(
          ...
          <h1 className="text-2xl font-bold text-gray-900 mb-2">
            Welcome to SisterRoam{user?.fullName ? `, ${user.fullName.split(' ')[0]}` : ''}! 🎉
          </h1>

          <div className="flex justify-center mb-6">
            <Badge variant="verified" size="md">Basic Member</Badge>
          </div>

          <p className="text-sm text-gray-500 mb-8">
            You're now part of a global community of women who travel and host with trust.
          </p>
              ...
              {actions.map(({ icon: Icon, title, desc, href }) => (
                ...
                >
                  <div className="w-9 h-9 rounded-lg bg-brand-lighter flex items-center justify-center shrink-0 group-hover:bg-brand group-hover:text-white transition-colors">
                    <Icon className="w-4.5 h-4.5 text-brand group-hover:text-white" aria-hidden="true" />
                  </div>
