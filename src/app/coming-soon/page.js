...
export default function ComingSoonPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-brand-lighter via-white to-pink/10 flex flex-col items-center justify-center px-4 py-12">
      ...
      {/* Brand name */}
      <h1 className="text-4xl font-bold text-brand mb-2 tracking-tight">
        SisterRoam
      </h1>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-10">
        {FEATURES.map(({ icon: Icon, text }) => (
          <div
            key={text}
            className="flex items-center gap-2.5 bg-white/70 backdrop-blur-sm border border-gray-100 rounded-2xl px-4 py-3 text-sm text-gray-700"
          >
        ...
        {!submitted ? (
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <input
                ...
                className="w-full px-4 py-3 border border-gray-200 rounded-2xl text-sm focus:outline-none focus:ring-0/30 focus:border-brand/40"
              />
              <button
                type="submit"
                disabled={loading}
          ...
          <div className="text-center space-y-3 py-4">
            ...
            <p className="text-sm text-gray-500">
              We&apos;ll email you at <strong>{email}</strong> when SisterRoam
              launches.
            </p>
            {count !== null && (
      ...
      <p className="mt-10 text-xs text-gray-400 text-center">
        SisterRoam © {new Date().getFullYear()} · Made with ❤️ for women who
        roam
      </p>
    </div>
  );
