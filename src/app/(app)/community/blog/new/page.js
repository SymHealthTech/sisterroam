...
export default function NewBlogPostPage() {
  ...
  useEffect(() => {
    if (status === 'authenticated') {
      ...
      if (!['verified', 'trusted'].includes(tier)) {
        toast.error('Verification required to write blog posts')
  ...
  async function save(isPublished) {
    ...
    try {
      ...
      router.push(`/community/blog/${d.data.slug}`)
    } finally {
      setter(false)
    }
  }
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 pb-24 space-y-5">
        ...
        <div className="border border-gray-200 rounded-2xl overflow-hidden">
          ...
          <div className="flex items-center gap-0.5 px-2 py-1.5 border-b border-gray-100 bg-gray-50/50">
            ...
            <ToolbarButton title="Heading 3" onClick={() => execCmd('formatBlock', 'H3')}>
              <Heading3 className="w-4 h-4" />
            </ToolbarButton>
            <div className="w-px h-5 bg-gray-200 mx-1" />
            <ToolbarButton title="Bullet list" onClick={() => execCmd('insertUnorderedList')}>
              <List className="w-4 h-4" />
            </ToolbarButton>
            <ToolbarButton title="Numbered list" onClick={() => execCmd('insertOrderedList')}>
              <ListOrdered className="w-4 h-4" />
            </ToolbarButton>
            <ToolbarButton title="Quote" onClick={() => execCmd('formatBlock', 'BLOCKQUOTE')}>
          ...
          <div
            ...
            className="min-h-[300px] p-4 text-sm text-gray-800 leading-relaxed focus:outline-none prose prose-sm max-w-none empty:before:content-[attr(data-placeholder)] empty:before:text-gray-300"
