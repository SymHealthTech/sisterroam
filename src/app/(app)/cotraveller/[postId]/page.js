  ...
  return (
    <div className="flex items-center gap-1.5">
      {Array.from({ length: max }).map((_, i) => (
        <span key={i} className={cn('w-3 h-3 rounded-full border-2', i < filled ? 'bg-brand border-brand' : 'bg-white border-gray-300')} />
      ))}
...
export default function TripDetailPage({ params }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!res.ok) { router.replace('/cotraveller'); return }
  ...
  async function handleApply(e) {
    ...
    try {
      ...
      const json = await res.json()
      if (!res.ok) { toast.error(json.error ?? 'Failed to send interest'); return }
      setSubmitted(true)
      setData(d => d ? { ...d, hasExpressedInterest: true, userInterestStatus: 'pending' } : d)
  ...
  const { post, authorProfile, hasExpressedInterest, userInterestStatus, chatRequestId } = data
  const author   = post.authorId ?? authorProfile ?? {}
  const isOwn    = author._id?.toString() === userId || post.authorId?._id?.toString() === userId || post.authorId?.toString() === userId
      ...
      <div className="max-w-3xl mx-auto px-4 py-5 space-y-5">
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="flex items-center gap-2 text-lg font-bold text-gray-900 flex-wrap">
                <span>{post.fromCity}{post.fromCountryCode && ` ${getFlagEmoji(post.fromCountryCode)}`}</span>
                <span className="text-gray-400 text-base">→</span>
                <span>{post.toCity}{post.toCountryCode && ` ${getFlagEmoji(post.toCountryCode)}`}</span>
              </div>
              <p className="text-sm text-gray-500 mt-1">{post.title}</p>
            </div>
            <span className={cn('text-xs px-2.5 py-1 rounded-full font-medium shrink-0', status.cls)}>
              {status.label}
            </span>
          </div>
          ...
          {(post.travelStyle ?? []).length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {post.travelStyle.map(s => (
                <span key={s} className="text-xs bg-brand-lighter text-brand px-2.5 py-0.5 rounded-full">
                  {CATEGORY_LABELS[s] ?? s}
                </span>
              ))}
            </div>
        ...
        <div className="flex gap-5 items-start flex-col lg:flex-row">
          ...
          <div className="flex-1 space-y-5 w-full">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-4 flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <p className="text-sm font-semibold text-gray-900">{author.fullName}</p>
                  {author.verificationTier === 'trusted' && <Badge variant="trusted">Trusted</Badge>}
                  {['verified', 'trusted'].includes(author.verificationTier) && <Badge variant="verified">✓ Verified</Badge>}
                </div>
                <p className="text-xs text-gray-500 mt-0.5">
                  {[author.city, author.country].filter(Boolean).join(', ')}
                  {author.createdAt && ` · Member since ${new Date(author.createdAt).getFullYear()}`}
                </p>
                ...
                {(author.averageRating > 0 || author.totalStays > 0) && (
                  <p className="text-xs text-gray-500 mt-1">
                    ...
                    {author.totalStays ?? 0} stay{author.totalStays !== 1 ? 's' : ''}
                  </p>
                )}
                <Link href={`/explore/${post.authorId?._id ?? post.authorId}`} className="text-xs text-brand hover:text-brand-dark font-medium mt-1 inline-block">
                  View full profile →
                </Link>
              </div>
            </div>
            ...
            {/* Description */}
            <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-4">
              <h3 className="text-sm font-semibold text-gray-800">About this trip</h3>
              <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-line">{post.description}</p>

              <div className="border-t border-gray-100 pt-4 space-y-2">
                <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Looking for</h4>
                <ul className="space-y-1.5 text-sm text-gray-600">
                  <li className="flex items-center gap-2">
                    <CheckCircle className="w-4 h-4 text-teal shrink-0" />
                    {post.lookingFor?.verifiedOnly ? 'Verified members preferred' : 'All members welcome'}
                  </li>
                  {(post.lookingFor?.minAge || post.lookingFor?.maxAge) && (
                    <li className="flex items-center gap-2">
                      <CheckCircle className="w-4 h-4 text-teal shrink-0" />
                      Age: {post.lookingFor.minAge ?? '18'} – {post.lookingFor.maxAge ?? 'any'}
                    </li>
                  )}
          ...
          <div className="w-full lg:w-80 shrink-0">
            {isOwn ? (
              <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-3">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Your trip post</p>
                <Button href={`/cotraveller/${postId}/interests`} variant="primary" fullWidth>
                  View interests ({post.interestedCount ?? 0})
                </Button>
                <Button href={`/cotraveller/${postId}/interests`} variant="ghost" fullWidth size="sm">
                  Manage trip
                </Button>
              </div>
            ) : !isOpen || isFull ? (
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-3">
                <p className="text-sm font-semibold text-gray-800">Your application</p>
                {userInterestStatus === 'pending' && (
                  <div className="flex items-center gap-2 p-3 bg-amber-lighter rounded-xl">
                    ...
                    <p className="text-sm text-amber-dark">Your interest is pending review</p>
                  </div>
                )}
                {userInterestStatus === 'accepted' && (
                  <div className="space-y-3">
                    <div className="flex items-center gap-2 p-3 bg-teal-lighter rounded-xl">
                      <CheckCircle className="w-4 h-4 text-teal shrink-0" />
                      <p className="text-sm text-teal-dark font-medium">You are matched!</p>
                    </div>
                    {chatRequestId && (
                      <Button href={`/messages/${chatRequestId}`} variant="primary" fullWidth>
                        Open chat
                      </Button>
                    )}
                  </div>
                ...
                {userInterestStatus === 'declined' && (
                  <div className="flex items-center gap-2 p-3 bg-gray-100 rounded-xl">
                    ...
                    <p className="text-sm text-gray-500">Not matched this time — keep exploring!</p>
                  </div>
                )}
              </div>
                ...
                <form onSubmit={handleApply} className="space-y-3">
                  ...
                  {submitted ? (
                    <div className="flex items-center gap-2 p-3 bg-teal-lighter rounded-xl">
                      ...
                      <p className="text-sm text-teal-dark">Interest sent! {author.fullName?.split(' ')[0]} will be in touch.</p>
                    </div>
                  ) : (
                    <Button type="submit" variant="primary" fullWidth loading={submitting} disabled={message.trim().length < 50}>
