...
function CheckinRow({ checkin, onConfirm }) {
  ...
  const confirmedStr = checkin.confirmedAt
    ? new Date(checkin.confirmedAt).toLocaleTimeString("en-US", {
        hour: "numeric",
        minute: "2-digit",
      })
    : null;
    ...
    <div className="flex items-center gap-3 py-3 border-b border-gray-50 last:border-0">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-gray-800">
          {TYPE_LABELS[checkin.checkinType]}
        </p>
...
export default function SafetyPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [reqJson, userJson] = await Promise.all([
        reqRes.json(),
        userRes.json(),
      ]);

      const active =
        reqJson.success && reqJson.data?.length ? reqJson.data[0] : null;
  ...
  async function handleSosActivate() {
    ...
    try {
      ...
      if (json.success) {
        ...
        toast.error(
          "Could not send SOS. Please call emergency services directly.",
        );
      }
    } catch {
      toast.error(
      ...
      <div className="max-w-[720px] mx-auto px-4 py-6 space-y-5">
            ...
            <div className="flex gap-0">
              ...
              <div className="flex-1 p-4">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="text-xs font-medium text-teal-dark uppercase tracking-wide">
                    Active stay
                  </p>
                </div>
                <div className="flex items-center gap-3 mt-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      {host?.city && `${host.city} · `}
                      {formatDateRange(
                        activeRequest.checkInDate,
                        activeRequest.checkOutDate,
                      )}
                    </p>
                  </div>
                </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          ...
          <p className="text-xs text-gray-500 mb-3 leading-relaxed">
            Hold the button for 3 seconds to send your location and an emergency
            alert to your emergency contact and the SisterRoam safety team.
          </p>
          ...
          {!loading && user && !user.emergencyContactEmail && (
            <div className="flex items-start gap-2.5 bg-amber-lighter rounded-xl p-3 mb-4">
              ...
              <p className="text-xs text-amber-dark leading-relaxed">
                No emergency contact email set — SOS will only notify the
                SisterRoam safety team.{" "}
                <button
                  onClick={() => {
                    setEmergencyOpen(true);
                    document
                      .getElementById("emergency-contact-section")
        ...
        >
          ...
          {!user?.emergencyContactName && !emergencyOpen && (
            <div className="flex items-start gap-3 bg-amber-lighter rounded-xl p-3 mb-4">
              <AlertTriangle className="w-4 h-4 text-amber shrink-0 mt-0.5" />
              <p className="text-xs text-amber-dark leading-relaxed">
                Add an emergency contact so we can alert them if you activate
                SOS.
              </p>
            </div>
          )}
            ...
            <form onSubmit={handleSaveContact} className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 text-sm border border-gray-200 rounded-xl
                             focus:outline-none focus:border-brand focus:ring-0/30  transition-colors"
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 text-sm border border-gray-200 rounded-xl
                             focus:outline-none focus:border-brand focus:ring-0/30 
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          ...
          <p className="text-xs text-gray-500 mb-4 leading-relaxed">
            All reports are reviewed by our safety team within 24 hours and
