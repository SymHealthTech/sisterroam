...
const COUNTRIES = [
  ...
  'Ecuador','Egypt','Ethiopia','Finland','France','Georgia','Germany','Ghana',
  'Greece','Guatemala','Hungary','India','Indonesia','Iran','Iraq','Ireland',
  'Israel','Italy','Jamaica','Japan','Jordan','Kazakhstan','Kenya','Kuwait',
  'Kyrgyzstan','Laos','Lebanon','Libya','Lithuania','Luxembourg','Malaysia',
  'Maldives','Malta','Mexico','Moldova','Mongolia','Morocco','Mozambique',
  ...
  return (
    <div ref={ref}>
      ...
      <button type="button" onClick={() => setOpen(v => !v)}
        className={cn('w-full h-[44px] sm:h-[40px] flex items-center justify-between px-3 rounded-lg border bg-white text-sm transition-colors',
...
function GenderCard({ value, label, emoji, selected, onClick }) {
  return (
    <button type="button" onClick={() => onClick(value)}
      className={cn('flex-1 flex flex-col items-center gap-1.5 py-4 px-3 rounded-xl border-2 transition-all text-sm font-medium',
        selected ? 'border-brand bg-brand-lighter text-brand' : 'border-gray-100 hover:border-gray-200 text-gray-600')}>
...
function Section({ title, children }) {
  return (
    <div className="bg-white border border-gray-100 rounded-2xl p-5 space-y-4">
      {title && <h2 className="text-sm font-semibold text-gray-900 pb-1 border-b border-gray-50">{title}</h2>}
      {children}
    </div>
  )
}
...
export default function ProfileEditPage() {
  ...
  useEffect(() => {
    fetch('/api/users')
      ...
      .then(d => {
        ...
        if (u.instagramUrl || u.linkedinUrl || u.facebookUrl) setShowSocial(true)
      })
  ...
  const buildPayload = useCallback(() => ({
    ...
    profilePhotoUrl, profilePhotoPublicId, fullName, age, gender, city, country,
    languages, education, occupation, bio, countriesVisited, categories, hobbies,
    instagram, linkedin, facebook, emergencyName, emergencyPhone, emergencyRel, emergencyEmail,
  ])
  ...
  function field(setter) {
    return (e) => { setter(e.target.value); triggerAutoSave() }
  }
    ...
    <AppLayout title="Edit Profile">
      <div className="max-w-2xl mx-auto px-4 py-6 pb-28 space-y-4">
        ...
        <Section title="Basic information">
          <Input label="Full name" value={fullName} onChange={field(setFullName)} placeholder="Your real name" required />
          <Input label="Age" type="number" value={age} onChange={field(setAge)} placeholder="e.g. 28" min={18} max={99} />
          <div>
            <label className="text-xs font-medium text-gray-600 mb-2 block">Gender</label>
            <div className="flex gap-3">
              <GenderCard value="female"     label="Female"     emoji="♀️"  selected={gender === 'female'}     onClick={v => { setGender(v); triggerAutoSave() }} />
              <GenderCard value="non-binary" label="Non-binary" emoji="⚧️"  selected={gender === 'non-binary'} onClick={v => { setGender(v); triggerAutoSave() }} />
              <GenderCard value="other"      label="Other"      emoji="🌈"  selected={gender === 'other'}      onClick={v => { setGender(v); triggerAutoSave() }} />
            </div>
          </div>
          <Input label="Home city"    value={city}    onChange={field(setCity)}    placeholder="e.g. Mumbai" />
          <SearchableSelect label="Home country" value={country} onChange={v => { setCountry(v); triggerAutoSave() }} options={COUNTRIES} placeholder="Select your country" />
        </Section>
        ...
        <Section title="About you">
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className={cn('text-[11px]', bio.length > 480 ? 'text-amber' : 'text-gray-400')}>{bio.length}/500</span>
            </div>
            <Textarea value={bio} rows={4} placeholder="Tell the community about yourself…" maxLength={500}
              onChange={e => { setBio(e.target.value); triggerAutoSave() }} />
          </div>
          <Select label="Education" value={education} onChange={e => { setEducation(e.target.value); triggerAutoSave() }}
            placeholder="Select level" options={EDUCATION_OPTIONS} />
          <Input label="Occupation" value={occupation} onChange={field(setOccupation)} placeholder="e.g. Software engineer" />
        </Section>

        {/* Travel */}
        <Section title="Travel profile">
          <TagInput label="Languages spoken" tags={languages}
            onChange={v => { setLanguages(v); triggerAutoSave() }}
            suggestions={COMMON_LANGUAGES} placeholder="Type a language and press Enter…" />
          <TagInput label="Countries visited" tags={countriesVisited}
            onChange={v => { setCountriesVisited(v); triggerAutoSave() }}
            suggestions={COUNTRIES} placeholder="Type a country and press Enter…" />
          <div>
              ...
              {TRAVELLER_CATEGORIES.map(({ value, label }) => (
                <button key={value} type="button" onClick={() => toggleCategory(value)}
                  className={cn('px-3 py-1.5 rounded-full text-sm font-medium border-2 transition-all',
                    categories.includes(value) ? 'border-brand bg-brand text-white' : 'border-gray-200 text-gray-600 hover:border-gray-300')}>
                  {label}
                </button>
              ))}
            </div>
          ...
          <div>
            <button type="button" onClick={() => setShowHobbies(v => !v)}
            ...
            {showHobbies && (
              <div className="mt-3">
                <TagInput tags={hobbies} onChange={v => { setHobbies(v); triggerAutoSave() }} placeholder="e.g. Photography, Yoga…" />
              </div>
            )}
          </div>
        </Section>
        ...
        <Section title="Emergency contact">
          <p className="text-xs text-gray-500">Used only during safety emergencies. Not shown publicly.</p>
          <Input label="Contact name"     value={emergencyName}  onChange={field(setEmergencyName)}  placeholder="Full name" />
          <Input label="Phone number"     value={emergencyPhone} onChange={field(setEmergencyPhone)} placeholder="+1 555 000 0000" />
          <Input label="Relationship"     value={emergencyRel}   onChange={field(setEmergencyRel)}   placeholder="e.g. Sister, Parent" />
          <Input label="Email (optional)" value={emergencyEmail} onChange={field(setEmergencyEmail)} placeholder="contact@email.com" type="email" />
        </Section>
        ...
        <Section>
          ...
          {showSocial && (
            <div className="space-y-3 mt-3">
              <Input label="Instagram" value={instagram} onChange={field(setInstagram)} placeholder="https://instagram.com/yourhandle" />
              <Input label="LinkedIn"  value={linkedin}  onChange={field(setLinkedin)}  placeholder="https://linkedin.com/in/yourname" />
              <Input label="Facebook"  value={facebook}  onChange={field(setFacebook)}  placeholder="https://facebook.com/yourname" />
            </div>
          )}
        </Section>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-4 py-4 z-10">
        <div className="max-w-2xl mx-auto flex gap-3">
          ...
          <Button fullWidth loading={saving} onClick={() => save(true)}>Save changes</Button>
        </div>
      </div>
    </AppLayout>
  )
