...
import {
  ...
  BedDouble, Users, BookOpen, PenSquare, MessageSquare,
  Star, Award, Plane, Clock,
} from 'lucide-react'
...
function getCompleteness(user, verifData) {
  ...
  const checks = [
    { label: 'Add a profile photo',     done: !!user.profilePhotoUrl,                           href: '/profile/edit' },
    { label: 'Write a bio (50+ chars)', done: (user.bio?.length ?? 0) >= 50,                    href: '/profile/edit' },
    { label: 'Add countries visited',   done: (user.countriesVisited?.length ?? 0) > 0,          href: '/profile/edit' },
    { label: 'Set your traveller style',done: (user.travellerCategories?.length ?? 0) > 0,       href: '/profile/edit' },
    { label: 'Add emergency contact',   done: !!user.emergencyContactName,                       href: '/profile/edit' },
    { label: 'Verify government ID',    done: verif?.status === 'approved' || verif?.status === 'pending', href: '/profile/verification' },
    { label: 'Record video intro',      done: !!verif?.selfieVideoUrl,                           href: '/profile/verification' },
  ]
...
const TIER_LABEL  = { basic: 'Basic',    verified: 'Verified', trusted: 'Trusted Sister' }
const TIER_BADGE  = { basic: 'basic',    verified: 'verified',  trusted: 'trusted' }
const ROLE_LABEL  = { guest: 'Traveller', host: 'Host',         both: 'Host & Traveller' }
    ...
    <div className="bg-white border border-gray-100 rounded-2xl p-4 space-y-3">
      <div className="flex items-start gap-3">
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <span className="text-sm font-semibold text-gray-900">{review.overallRating}</span>
        </div>
      </div>
...
function HostingTab({ host, onToggle }) {
  if (!host) return (
    <div className="text-center py-12">
      ...
      <p className="text-sm text-gray-500 mb-4">You haven't set up a host listing yet.</p>
  ...
  const TYPE_LABELS = {
    ...
    couch: 'Couch', floor_space: 'Floor space', tent_space: 'Tent space',
  }
  const OFFERING_LABELS = {
    bed: 'Bed', breakfast: 'Breakfast', dinner: 'Home dinner',
    city_guide: 'City tour', airport_pickup: 'Airport pickup',
    laundry: 'Laundry', wifi: 'Wi-Fi', bicycle: 'Bicycle',
  }
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-4 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <Link href="/profile/host-listing" className="text-xs text-brand font-medium hover:underline flex items-center gap-0.5">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div>
            ...
            <p className="font-medium text-gray-900">{TYPE_LABELS[host.accommodationType] ?? host.accommodationType}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {host.freeOfferings.map(o => (
                <span key={o} className="px-2 py-0.5 bg-teal-lighter text-teal text-xs rounded-full font-medium">
                  {OFFERING_LABELS[o] ?? o}
                </span>
              ))}
            </div>
        ...
        {host.houseRules && (
          <div>
            ...
            <p className="text-sm text-gray-600 leading-relaxed line-clamp-3">{host.houseRules}</p>
          </div>
        )}
      </div>
    </div>
...
export default function ProfilePage() {
  ...
  const fetchReviews = useCallback(async (page = 1) => {
    ...
    try {
      const res  = await fetch(`/api/reviews?revieweeId=${user._id}&page=${page}&limit=5`)
  ...
  useEffect(() => {
    if (activeTab === 'reviews' && user?._id && reviews.length === 0) {
      fetchReviews(1)
    }
  }, [activeTab, user?._id])
  ...
  async function handleHostToggle(field, value) {
    ...
    try {
      ...
      if (!res.ok) { setHost(prev); toast.error('Failed to update') }
    } catch {
      setHost(prev)
  ...
  if (loading) return <AppLayout title="Profile"><ProfileSkeleton /></AppLayout>
  if (!user) return null
    ...
    <AppLayout title="My Profile">
      <div className="max-w-5xl mx-auto px-4 py-6 lg:px-6">
              ...
              <div className="flex flex-col items-center text-center lg:flex-row lg:items-start lg:text-left lg:gap-5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center justify-center lg:justify-start gap-2 mt-2">
                    <Badge variant={TIER_BADGE[user.verificationTier ?? 'basic']}>
                      <Shield className="w-3 h-3" />
                      {TIER_LABEL[user.verificationTier ?? 'basic']}
                    </Badge>
                    <Badge variant="basic">{ROLE_LABEL[user.role ?? 'guest']}</Badge>
                  </div>
                  ...
                  {/* Social links */}
                  {(user.instagramUrl || user.linkedinUrl || user.facebookUrl) && (
                    <div className="flex items-center justify-center lg:justify-start gap-3 mt-3">
                      {user.instagramUrl && (
                        <a href={user.instagramUrl} target="_blank" rel="noopener noreferrer"
                          className="text-gray-400 hover:text-pink transition-colors" aria-label="Instagram">
                          <Globe className="w-4 h-4" />
                        </a>
                      )}
                      {user.linkedinUrl && (
                        <a href={user.linkedinUrl} target="_blank" rel="noopener noreferrer"
                          className="text-gray-400 hover:text-brand transition-colors" aria-label="LinkedIn">
                          <Globe className="w-4 h-4" />
                        </a>
                      )}
                      {user.facebookUrl && (
                        <a href={user.facebookUrl} target="_blank" rel="noopener noreferrer"
                          className="text-gray-400 hover:text-brand transition-colors" aria-label="Facebook">
                          <Globe className="w-4 h-4" />
                        </a>
                      )}
                    </div>
                ...
                <div className="flex items-center justify-center gap-6 mt-4 lg:mt-0 lg:shrink-0 lg:flex-col lg:items-end lg:gap-3">
                  <div className="text-center lg:text-right">
                    <p className="text-lg font-bold text-gray-900">{user.totalStays ?? 0}</p>
                    <p className="text-xs text-gray-400">Stays</p>
                  </div>
                  <div className="text-center lg:text-right">
                    <p className="text-lg font-bold text-gray-900">{user.totalReviews ?? 0}</p>
                ...
                {activeTab === 'about' && (
                  <div className="space-y-5">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {user.languages.map(l => (
                            <span key={l} className="px-2.5 py-1 bg-brand-lighter text-brand text-xs rounded-full font-medium">
                              {l}
                            </span>
                          ))}
                        </div>
                    ...
                    {user.travellerCategories?.length > 0 && (
                      <div>
                        <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Travel style</h3>
                        <div className="flex flex-wrap gap-1.5">
                    ...
                    {user.hobbies?.length > 0 && (
                      <div>
                        <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Hobbies & interests</h3>
                        <div className="flex flex-wrap gap-1.5">
                          {user.hobbies.map(h => (
                    ...
                    {user.averageRating > 0 && (
                      <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl">
                        <div className="text-center">
                          <p className="text-3xl font-bold text-gray-900">{user.averageRating.toFixed(1)}</p>
                          <div className="flex items-center justify-center gap-0.5 mt-1">
                            {[1,2,3,4,5].map(s => (
                              <Star key={s} className={cn('w-3.5 h-3.5', s <= Math.round(user.averageRating) ? 'fill-amber text-amber' : 'text-gray-200')} />
                            ))}
                          </div>
                          <p className="text-xs text-gray-400 mt-1">{user.totalReviews} review{user.totalReviews !== 1 ? 's' : ''}</p>
                        </div>
                      </div>
                    )}
                    {reviewsLoading && reviews.length === 0 ? (
                      <div className="space-y-3">
                        {[1,2].map(i => <Skeleton key={i} variant="card" className="h-28" />)}
                      </div>
                    ) : reviews.length === 0 ? (
          ...
          <div className="hidden lg:flex flex-col gap-4 mt-0">
              ...
              {missing.length > 0 ? (
                <ul className="space-y-2">
                  {missing.map(({ label, href }) => (
                    <li key={label}>
                      <Link href={href} className="flex items-center gap-2 text-xs text-gray-600 hover:text-brand transition-colors group">
            ...
            <div className="bg-white border border-gray-100 rounded-2xl p-4 space-y-2">
              ...
              {[
                ...
                { label: 'Verification',        href: '/profile/verification', icon: Shield },
                { label: isHost ? 'Host listing' : 'Become a host', href: '/profile/host-listing', icon: BedDouble },
        ...
        <div className="lg:hidden mt-4 bg-white border border-gray-100 rounded-2xl p-4">
          ...
          {missing.slice(0, 3).map(({ label, href }) => (
            <Link key={label} href={href} className="flex items-center gap-2 text-xs text-gray-500 py-1.5 hover:text-brand">
      ...
      {photoModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black/40" onClick={() => setPhotoModal(false)} />
          <div className="relative bg-white rounded-2xl p-6 max-w-sm w-full mx-4 space-y-4">
            <h2 className="font-semibold text-gray-900">Update profile photo</h2>
            <ImageUpload
