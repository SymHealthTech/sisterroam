...
import { Search, MapPin, MessageSquare, CheckCircle, ThumbsUp, ChevronUp, ChevronDown, PlusCircle } from 'lucide-react'
...
function QuestionCard({ question, userId, isVerified, verifPending, verifApproved, onAnswered }) {
  ...
  async function handleAnswer(e) {
    e.preventDefault()
    if (answerText.trim().length < 30) { toast.error('Answer must be at least 30 characters'); return }
    setSubmitting(true)
    try {
      const res = await fetch(`/api/recommendations/questions/${question._id}`, {
        method: 'POST',
  ...
  async function handleMarkAccepted(answerId) {
    ...
    if (res.ok) {
      setAnswers(a => a?.map(ans => ({ ...ans, isAccepted: ans._id === answerId })))
  ...
  async function handleUpvoteAnswer(answerId) {
    ...
    if (res.ok) {
      const data = await res.json()
      setAnswers(a => a?.map(ans => ans._id === answerId
        ? { ...ans, upvoteCount: data.data.upvoteCount, hasUpvoted: data.data.upvoted }
        : ans
      ))
    }
  }

  const isMyQuestion = author._id?.toString() === userId || author.toString?.() === userId
    ...
    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
      ...
      <div className="p-4 space-y-2">
        ...
        <div className="flex items-start gap-1.5">
          ...
          <span className="text-xs text-gray-500">{question.city}, {question.country}</span>
          {question.category && (
            <span className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full ml-1">{question.category}</span>
          )}
        </div>
        ...
        <div className="flex items-center gap-3 text-xs text-gray-400">
          <span>{question.answersCount ?? 0} answer{question.answersCount !== 1 ? 's' : ''}</span>
          <span>{question.viewsCount ?? 0} views</span>
          <button type="button" onClick={toggleExpand} className="ml-auto flex items-center gap-0.5 text-brand hover:text-brand-dark font-medium">
            {expanded ? <>Hide answers <ChevronUp className="w-3.5 h-3.5" /></> : <>Answer <ChevronDown className="w-3.5 h-3.5" /></>}
          </button>
        </div>
      </div>
      ...
      {expanded && (
        <div className="border-t border-gray-100 bg-gray-50">
          {loading && <p className="text-xs text-gray-400 text-center py-4">Loading answers...</p>}

          {(answers ?? []).map(ans => {
            const ansAuthor = ans.authorId ?? {}
            return (
              <div key={ans._id} className={cn('px-4 py-3 border-b border-gray-100 last:border-b-0', ans.isAccepted && 'bg-teal-lighter/30')}>
                {ans.isAccepted && (
                ...
                <div className="flex items-center gap-2 mb-1.5">
                  <Avatar src={ansAuthor.profilePhotoUrl} name={ansAuthor.fullName} size="xs" />
                  <span className="text-xs font-medium text-gray-700">{ansAuthor.fullName}</span>
                  {['verified', 'trusted'].includes(ansAuthor.verificationTier) && <Badge variant="verified" size="sm">✓</Badge>}
                  <span className="text-[10px] text-gray-400">{formatRelativeTime(ans.createdAt)}</span>
                </div>
                ...
                <div className="flex items-center justify-between mt-2">
                  <button
                    ...
                    className={cn('flex items-center gap-1 text-xs', ans.hasUpvoted ? 'text-brand' : 'text-gray-400 hover:text-brand')}
                  >
                    <ThumbsUp className={cn('w-3.5 h-3.5', ans.hasUpvoted && 'fill-brand')} />
...
export default function RecommendationsPage() {
  ...
  const fetchRecs = useCallback(async (p = 1, cat = category, s = sort, q = search) => {
    setLoading(true)
    try {
      ...
      if (!res.ok) return
      const data = await res.json()
      if (p === 1) setRecs(data.data?.recommendations ?? [])
      else         setRecs(prev => [...prev, ...(data.data?.recommendations ?? [])])
      setRecTotal(data.data?.total ?? 0)
      setRecPage(p)
    } finally {
      setLoading(false)
    }
  ...
  const fetchQs = useCallback(async (p = 1) => {
    setLoading(true)
    try {
      ...
      if (!res.ok) return
      const data = await res.json()
      if (p === 1) setQuestions(data.data?.questions ?? [])
      else         setQuestions(prev => [...prev, ...(data.data?.questions ?? [])])
      setQTotal(data.data?.total ?? 0)
      setQPage(p)
    } finally {
      setLoading(false)
    }
  ...
  useEffect(() => {
    if (activeTab === 0) fetchRecs(1, category, sort, search)
    if (activeTab === 1) fetchQs(1)
    if (activeTab === 2) {
      fetchRecs(1, '', sort, '')
      fetchQs(1)
    }
  ...
  function handleUpvote(id, upvoted, count) {
    setRecs(prev => prev.map(r => r._id === id ? { ...r, hasUpvoted: upvoted, upvoteCount: count } : r))
  }
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
              {CATEGORIES.map(({ value, label }) => (
                <button
                  ...
                  onClick={() => {
                    setCategory(value)
                    if (activeTab === 0) fetchRecs(1, value, sort, search)
                    if (activeTab === 1) fetchQs(1)
                  }}
                  className={cn(
              ...
              <div className="flex gap-2">
                {SORT_OPTIONS.map(({ value, label }) => (
                  <button
                    key={value}
                    type="button"
                    onClick={() => { setSort(value); fetchRecs(1, category, value, search) }}
                    className={cn(
                      'px-3 py-1.5 rounded-full text-xs border transition-colors',
                      sort === value ? 'bg-gray-900 text-white border-gray-900' : 'border-gray-200 text-gray-600 hover:border-gray-400'
                    )}
                  >
                    {label}
                  </button>
          ...
          <>
            {!loading && (
              <p className="text-xs text-gray-500">
                {search ? `${recTotal} recommendation${recTotal !== 1 ? 's' : ''} for "${search}"` : `${recTotal} recommendation${recTotal !== 1 ? 's' : ''}`}
              </p>
            )}
            {loading && recs.length === 0 ? (
              <div className="space-y-3">
                {[1, 2, 3].map(i => <RecommendationCard.Skeleton key={i} />)}
              </div>
            ) : recs.length > 0 ? (
              <>
                <div className="space-y-3">
                  {recs.map(rec => (
                    <RecommendationCard
                      ...
                      canEdit={rec.authorId?._id === userId || rec.authorId === userId}
                    />
                  ))}
                </div>
                {recs.length < recTotal && (
                  <div className="flex justify-center">
                    <Button variant="ghost" size="sm" loading={loading} onClick={() => fetchRecs(recPage + 1, category, sort, search)}>
                      Load more
                    </Button>
                  </div>
                )}
            ...
            {loading && questions.length === 0 ? (
              ...
              <>
                ...
                {questions.length < qTotal && (
                  <div className="flex justify-center">
                    <Button variant="ghost" size="sm" loading={loading} onClick={() => fetchQs(qPage + 1)}>
                      Load more
                    </Button>
                  </div>
                )}
              ...
              <div className="text-center py-12 space-y-3">
                ...
                <Button variant="primary" size="sm" onClick={() => setQModal(true)}>Ask the first question</Button>
              </div>
            )}
          </>
        )}
          ...
          <div className="space-y-6">
            <section>
              <h3 className="text-sm font-semibold text-gray-700 mb-3">
                My recommendations ({recs.filter(r => r.authorId?._id === userId || r.authorId === userId).length})
              </h3>
              <div className="space-y-3">
                {recs.filter(r => r.authorId?._id === userId || r.authorId === userId).map(rec => (
                  <RecommendationCard key={rec._id} rec={rec} onUpvote={handleUpvote} canEdit />
                ))}
              </div>
              {recs.filter(r => r.authorId?._id === userId || r.authorId === userId).length === 0 && (
            ...
            <section>
              <h3 className="text-sm font-semibold text-gray-700 mb-3">
                My questions ({questions.filter(q => q.authorId?._id === userId || q.authorId === userId).length})
              </h3>
              <div className="space-y-3">
                {questions.filter(q => q.authorId?._id === userId || q.authorId === userId).map(q => (
                  <QuestionCard key={q._id} question={q} userId={userId} />
                ))}
              </div>
              {questions.filter(q => q.authorId?._id === userId || q.authorId === userId).length === 0 && (
